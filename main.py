...
def print_table_zakup_brutto():
    temp_dict = create_table_from_zakup_brutto(zakup_brutto= st.session_state.zakup_brutto_value, VAT=st.session_state.VAT/100, marza=st.session_state.marza_value/100)
    temp_df = pd.DataFrame.from_dict(temp_dict, orient='index', columns=['Wartości']).transpose()
    st.data_editor(
        temp_df,
        column_order=['zakup netto', 'zakup brutto', 'VAT', 'marża brutto', 'zysk', 'cena netto', 'cena brutto'],
        column_config={
...
def print_table_cena_brutto():
    temp_dict = create_table_from_cena_brutto(cena_brutto= st.session_state.cena_brutto_value, VAT=st.session_state.VAT/100, marza=st.session_state.marza_value/100)
    temp_df = pd.DataFrame.from_dict(temp_dict, orient='index', columns=['Wartości']).transpose()
    st.data_editor(
        temp_df,
        column_order=['zakup netto', 'zakup brutto', 'VAT', 'marża brutto', 'zysk', 'cena netto', 'cena brutto'],
...
with zakup_netto:
    zakup_netto = st.number_input("Wprowadź cenę zakupu netto", step=0.01, key='zakup_netto_value', on_change=print_table_zakup_netto)
with zakup_brutto:
